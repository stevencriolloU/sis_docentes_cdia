                    ...
                    <div class="w-full">
                        <div class="bg-gray-500 rounded-lg p-6 shadow flex flex-col justify-center items-center">
                            <div class="flex items-center justify-between sm:flex-auto">
                                <h1 class="mt-2 font-semibold text-white">Lista de {{ __('Cursos') }} Disponibles</h1>
                                <img src="{{ asset('images/cursos.svg') }}" alt="Icono de inicio de sesión" class="h-8 w-8 ml-4">
                            </div>
                        </div>
                            ...
                            <div class="mt-8 overflow-x-auto text-center">
                                <div class="inline-block min-w-full py-2 text-center rounded-lg align-middle bg-gray-500">
                                    <table class="w-full divide-y divide-gray-300 text-center">
                                        <thead>
                                        <tr>
                                            <th scope="col" class="py-3 pl-4 pr-3 text-xs font-semibold uppercase tracking-wide text-white">No</th>
                                            <th scope="col" class="py-3 pl-4 pr-3 text-xs font-semibold uppercase tracking-wide text-white">Semestres</th>
                                            <th scope="col" class="py-3 pl-4 pr-3 text-xs font-semibold uppercase tracking-wide text-white">Paralelos</th>
                                            <th scope="col" class="px-3 py-3 text-xs font-semibold uppercase tracking-wide text-white">Acciones</th>
                                        </tr>
                                        </thead>
                                        <tbody class="divide-y divide-gray-200 bg-white item-center">
                                        @foreach ($cursos as $curso)
                                            <tr class="even:bg-gray-50">
                                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-l font-semibold text-gray-900">{{ ++$i }}</td>
                                                <td class="whitespace-nowrap px-3 py-4 text-l text-gray-500">{{ $curso->semestre }}</td>
                                                <td class="whitespace-nowrap px-3 py-4 text-l text-gray-500">{{ $curso->paralelo }}</td>
                                                <td class="whitespace-nowrap py-4 pl-4 pr-5 text-center text-sm font-medium text-gray-900">
                                                    <form action="{{ route('cursos.destroy', $curso->id) }}" method="POST" class="flex justify-center">
